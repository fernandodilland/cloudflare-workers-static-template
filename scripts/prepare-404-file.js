...
try {
  if (!fs.existsSync(html404)) {
    if (fs.existsSync(folder404Index)) {
      ...
      console.log('404.html preparado correctamente.');
    } else {
      console.log('No se encontró src/404/index.html, no se realizaron cambios.');
    }
  } else {
    console.log('src/404.html ya existe, no se realizaron cambios.');
  }
} catch (err) {
  console.error('Error preparando 404.html:', err);
  process.exit(1);
}
